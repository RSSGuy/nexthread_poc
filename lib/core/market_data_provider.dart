...
class MarketDataProvider {
  ...
  // --- NEW: GLOBAL BENCHMARKS (Data Add-on) ---
  Future<MarketFact> getGlobalBenchmarks() async {
    List<MarketFact> facts = [];

    // Iterate over the static global list
    for (var entry in SectorBenchmarks.globalIndices.entries) {
      // Use Finance as the fallback category since these are indices
  ...
  MarketFact _generateSimulatedFact(String ticker, Naics industry, {String? label}) {
    ...
    double basePrice = isTech ? 15000.0 : 4000.0; // Adjusted base for indices
    if (ticker.contains("X")) basePrice = 100.0; // ETFs
...
class MarketDataProvider {
  ...
  // --- PERSISTENT GLOBAL ANALYSIS CACHE ---

  void cacheGlobalAnalysis(String analysis) {
    // Fire and forget save to Hive
    StorageService.saveGlobalAnalysis(analysis);
  }

  (String?, DateTime?) getCachedGlobalAnalysis() {
...
import 'models.dart';
import 'sector_benchmarks.dart';
import 'storage_service.dart'; // REQUIRED IMPORT
...
class MarketDataProvider {
  ...
  void cacheGlobalAnalysis(String analysis) {
    // Saves to Hive via StorageService
    StorageService.saveGlobalAnalysis(analysis);
  }

  // Returns (Analysis Content, Timestamp)
  (String?, DateTime?) getCachedGlobalAnalysis() {
    final data = StorageService.getGlobalAnalysis();
    if (data != null) {
      return (
