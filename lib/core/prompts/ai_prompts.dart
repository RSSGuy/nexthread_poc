...
''';
  ...
  static String generateDynamicSectorAnalysis({
    ...
    final roleName = roleConfig['role'] ?? 'Investigative Industry Analyst';

    // Safely handle both the old Map format and the new String format for instructions
    String roleInstructions = 'Provide a deep, compelling analysis.';
    if (roleConfig['instructions'] is String) {
      roleInstructions = roleConfig['instructions'];
    } else if (roleConfig['instructions'] is Map) {
      roleInstructions = "Provide a deep, compelling analysis adhering to standard parameters.";
    }

    return '''
You are acting as a "$roleName". 
Your instructions: $roleInstructions
$_jsonFormattingRule
$_dateContext

Your task is to write a highly compelling, verbose, and journalistic analysis for the sector: $sectorName.
Base your narrative strictly on the provided news context and market data.

CRITICAL STYLE GUIDELINES:
1. Write in the style of a premium financial news feature (e.g., Bloomberg, Wall Street Journal).
2. BE VERBOSE. Write flowing, multi-paragraph narratives that weave the facts into a compelling story. 
3. Emphasize the stakes, the scale of the money involved, and the human/economic impact.

CONTEXT TO ANALYZE:
${sectorNews.join("\n")}

OUTPUT FORMAT:
You MUST return a valid JSON object with the following keys. Do NOT wrap the JSON in markdown formatting block quotes, just return the raw JSON.
{
  "sector_name": "$sectorName",
  "synthesized_development": "Write your main feature article here. Start with a Catchy Title. Use double line breaks (\\\\n\\\\n) for paragraphs. Use ALL CAPS for your subheadings to break up the text (e.g., \\\\n\\\\nTHE VERDICT\\\\n\\\\n). Write at least 4-5 rich paragraphs.",
  "strategic_insight": "A sharp, analytical 'Bottom Line' takeaway. Why does this overarching narrative matter to investors or policymakers?",
  "opportunity": "Actionable, forward-looking advice based on the narrative.",
  "visual_suggestion": "A detailed prompt for an editorial illustration capturing this narrative.",
  "ai_logic_references": ["List the exact headlines, news sources, or quotes from the CONTEXT TO ANALYZE that you used to form this analysis."]
}
''';
  }


  // --- 7A. SECTOR-SPECIFIC ANALYSIS (Fallback / Direct Call) ---
  static String industrialSectorAnalysisSystem(String sectorName, List<String> sectorNews) {
    return '''
You are a Senior Industrial Strategy Consultant acting as a premier financial journalist focusing exclusively on the "$sectorName" sector.
$_jsonFormattingRule
$_dateContext
...
INSTRUCTIONS & ANALYSIS PARAMETERS:
1. Write a highly detailed, verbose feature article specifically for the "$sectorName" sector.
2. VERBOSITY REQUIREMENT: You MUST write flowing, multi-paragraph narratives. Do not use bullet points.

CRITICAL STYLE GUIDELINES:
1. Write this as a front-page Editorial Feature or Op-Ed (e.g. Wall Street Journal style). 
2. BE VERBOSE AND COMPELLING. Connect the dots between the different news items. Find the overarching macro-economic or geopolitical narrative.
3. Use a commanding, insightful, and authoritative journalistic tone.

OUTPUT JSON FORMAT:
{
  "sector_name": "$sectorName",
  "synthesized_development": "Write your main feature article here. Start with a Catchy Title. Use double line breaks (\\\\n\\\\n) for paragraphs. Use ALL CAPS for your subheadings to break up the text. At least 4-5 paragraphs.",
  "strategic_insight": "Write a highly detailed paragraph explaining the 'so what' factor and cascading consequences for markets.",
  "opportunity": "Write a highly detailed paragraph outlining a specific, actionable upside or strategic pivot.",
  "visual_suggestion": "Write a detailed paragraph describing a specific chart, graph, or illustration.",
  "ai_logic_references": ["List the exact headlines, news sources, or quotes from the SECTOR NEWS that you used to form this analysis."]
}
''';
  }
  ...
  static String industrialConclusionSystem(List<String> sectorSummaries) {
    return '''
You are the Editor-in-Chief of a premium industrial strategy publication.
$_jsonFormattingRule

You have just received the following sector-by-sector feature articles from your journalists:

[SECTOR SUMMARIES]
${sectorSummaries.join('\n\n')}

TASK:
Review the developments across all the analyzed sectors above. Write the "Executive Synthesis & Meta-Trend" conclusion.

CRITICAL STYLE GUIDELINES:
1. Write this as a front-page Editorial Op-Ed. 
2. BE VERBOSE AND COMPELLING. Connect the dots between the different sectors. Find the overarching macro-economic or geopolitical narrative tying them together.
3. Write 3 to 4 rich, flowing paragraphs. Use a commanding, insightful, and authoritative journalistic tone.

OUTPUT JSON FORMAT:
{
  "synthesis_conclusion": "Your highly compelling, multi-paragraph editorial synthesis here. Use double line breaks (\\\\n\\\\n) to separate paragraphs."
}
''';
  }

  // --- 8. DYNAMIC JSON ROLE BUILDER (Private) ---
  // Left completely intact to ensure any legacy parts of the app relying on this do not break.
  static String _buildDynamicRoleSystem({
    required Map<String, dynamic> roleConfig,
    required List<String> newsData,
    String? specificSector,
