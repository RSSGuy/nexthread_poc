...
class AIService {
  ...
  // --- MAIN GENERATION ---
  Future<void> generateBriefing(TopicConfig topic, {
    List<String>? manualFeedPaths, // CHANGED: Now accepts a List
    String? customScenario,
    List<TopicConfig>? allTopics
  }) async {
...
/*  Future<Map<String, dynamic>> _generateInternal(
    ...
    if (forceCrossSectorAnalysis) {
      crossSectorInstruction = '''
        [ATTENTION: CROSS-SECTOR DATA INJECTED]
        I have provided headlines from OTHER sectors labeled [SECTOR: NAME].
        TASK: Explicitly add a paragraph starting with "Cross-Sector Observations:" in the summary.
        ''';
    }

    final systemPrompt = '''
      You are an Intelligence Analyst for the ${topic.name} sector.
      [GLOBAL MACRO CONTEXT] $globalContext
      [SECTOR SPECIFIC DATA] ${marketFact.toString()}
      [NEWS STREAM] ${news.join('\n')}
      [ANALYSIS RULES] ${topic.riskRules}
      $scenarioBlock
      $crossSectorInstruction
      OUTPUT JSON: { "briefs": [{ "id", "subsector", "title", "summary", "severity", "fact_score", "sent_score", "divergence_tag", "divergence_desc", "metrics":{commodity,price,trend}, "chart_data":[], "headlines":[], "is_fallback":false }] }
      ''';
