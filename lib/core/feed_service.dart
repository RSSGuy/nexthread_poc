      ...
      try {
        ...
        if (body != null && body.isNotEmpty) {
          ...
          for (var rawItem in items) {
            // Split out the URL so we only keyword-match against the title
            final parts = rawItem.split(' | URL: ');
            final t = parts[0].replaceAll(RegExp(r'\s+'), ' ').trim();
            final url = parts.length > 1 ? parts[1] : "No URL";

            if (t.length < 5) continue;

            if (keywords.any((k) => t.toLowerCase().contains(k.toLowerCase()))) {
              headlines.add("[${source.name}] $t | URL: $url");
            }
          }
        }
      } catch (e) {
    ...
    try {
      ...
      if (atom.items != null) {
        return atom.items!.map((item) {
          final title = item.title ?? "";
          final link = (item.links != null && item.links!.isNotEmpty) ? item.links!.first.href : "No URL";
          return "$title | URL: $link";
        }).toList();
      }
    } catch (_) {}
