...
// lib/core/local_feed_service.dart
...
class LocalFeedService {
  ...
  /// Loads headlines from multiple paths and aggregates them.
  Future<List<String>> getHeadlinesFromPaths(List<String> filePaths, List<String> keywords) async {
    List<String> allHeadlines = [];
    for (var path in filePaths) {
      final headlines = await getHeadlinesFromPath(path, keywords);
...
class LocalFeedService {
  ...
  /// Specifically parses feeds for cross-sector intelligence, retaining Category, Description, and URLs
  Future<List<String>> getCrossSectorIntelligence(String filePath) async {
    try {
      final xmlString = await rootBundle.loadString(filePath);
      List<String> formattedItems = [];
      ...
      try {
        ...
        if (rss.items != null) {
          for (var item in rss.items!) {
            ...
            // --- EXTRACT THE LINK ---
            final link = item.link?.trim() ?? "No Link Available";

            // Format it specifically for the AI to understand the sector context AND the URL
            formattedItems.add("[SECTOR: $category] $title - $description | URL: $link");
          }
        }
      } catch (e) {
        print("Error parsing Cross-Sector RSS: $e");
