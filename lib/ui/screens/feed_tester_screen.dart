...
import '../../core/news_registry.dart';
import '../../core/feed_service.dart';
import '../../core/storage_service.dart'; // Import StorageService
...
class _FeedTesterScreenState extends State<FeedTesterScreen> {
  ...
  bool _isLoading = false;
  List<FeedHealthResult> _results = [];
  Map<String, FeedHealthResult> _resultsMap = {};
  ...
  Future<void> _loadPreviousResults() async {
    final data = StorageService.getLastFeedHealthResults();
    if (data != null) {
      if (!mounted) return;
      setState(() {
        _lastRunDate = data['timestamp'];
        _results = data['results'] as List<FeedHealthResult>;
        // Rebuild map
  ...
  Future<void> _runFullAudit() async {
    ...
    // Save Results
    await StorageService.saveFeedHealthResults(_results);
    setState(() {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Audit Complete & Saved.")),
      );
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("RSS Health Check", style: TextStyle(fontSize: 18)),
            if (_lastRunDate != null)
              Text(
                "Last Run: ${_lastRunDate!.toLocal().toString().split('.')[0]}",
      ...
      body: Column(
        children: [
          ...
          if (_isLoading && _results.isEmpty)
            const Expanded(child: Center(child: CircularProgressIndicator())),
          Expanded(
            child: displayedResults.isEmpty && !_isLoading
