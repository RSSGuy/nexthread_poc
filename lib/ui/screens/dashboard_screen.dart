...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  MarketFact? _marketFact;
  bool _loading = false;
  int _userPoints = 0; // Tracks the user's gamification balance
  ...
  // Filter topics based on selected industry tab
  List<TopicConfig> get _filteredTopics {
    return _allTopics.where((t) => t.industry == _selectedIndustry).toList();
  }
  ...
  // Helper to refresh points from child widgets (e.g., after a comment payment)
  void _refreshPoints() {
    setState(() {
      _userPoints = StorageService.getPoints();
    });
  ...
  void _generateNewBriefing(MarketFact? fact, {String? manualFeedPath, String? customScenario}) async {
    // 1. DEDUCT FULL COST UPFRONT (1000 pts)
    if (customScenario != null) {
      bool success = await StorageService.deductPoints(1000);
      if (!success) {
        if (mounted) {
    ...
    try {
      ...
      // 4. GUARDRAIL BLOCKED: APPLY PENALTY
      // We already deducted 1000.
      // Requirement: "If rejected, there should still be a cost of 500 points."
      // Therefore, we refund 500 points. (1000 paid - 500 refund = 500 cost).
      await StorageService.addPoints(500);

      if (mounted) {
        setState(() {
          _userPoints = StorageService.getPoints(); // Update UI
          _loading = false;
        });
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    const Text("Simulation Rejected", style: TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
                const SizedBox(height: 4),
                Text("Reason: ${e.message}", style: const TextStyle(fontSize: 12)),
                ...
                Container(
                  ...
                  child: const Text("PENALTY APPLIED: 500 pts deducted", style: TextStyle(color: Colors.amberAccent, fontWeight: FontWeight.bold, fontSize: 11)),
                ),
              ],
            ),
      ...
      // 5. SYSTEM ERROR: FULL REFUND
      print("Error generating briefing: $e");

      if (customScenario != null) {
        await StorageService.addPoints(1000); // Give back everything on technical failure
        if (mounted) setState(() => _userPoints = StorageService.getPoints());
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("System Error (Points Refunded): $e"),
  ...
  void _performSystemReset() async {
    ...
    _initLoad(); // Will reset points to default 5000
  ...
  void _onModelSelect() async {
    ...
    if (providerKey != null) {
      _aiService.setProvider(providerKey);
      setState(() {}); // Rebuild to update menu text
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          // 2. MARKET PULSE & SIMULATION TRIGGER
          if (_filteredTopics.isNotEmpty)
            MarketPulseCard(
              topics: _filteredTopics,
              currentTopic: _currentTopic,
          ...
          Expanded(
            child: _loading && _briefings.isEmpty
              ...
              itemBuilder: (context, index) => BriefingCard(
                ...
                industryTag: _currentTopic.industry.label.split(',')[0],
                topicId: _currentTopic.id,
                // NEW: Pass refresh callback to allow child interactions to update points
                onPointsUpdated: _refreshPoints,
              ),
            ),
          ),
...
import '../dialogs/fallback_selector_dialog.dart';
import '../dialogs/model_selector_dialog.dart';
import '../dialogs/industry_selector_dialog.dart'; // RESTORED IMPORT
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // --- RESTORED: Handle Industry Button Click ---
  void _onIndustrySelectorClicked() async {
    final Set<Naics> availableIndustries = _allTopics.map((t) => t.industry).toSet();
  ...
  @override
  Widget build(BuildContext context) {
    // Keep this calculation for the handler, though unused in build directly now
    // final Set<Naics> availableIndustries = _allTopics.map((t) => t.industry).toSet();
    ...
    return Scaffold(
      ...
      body: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // 1. INDUSTRY SELECTOR (RESTORED - Replaces TopicFilterBar)
          Padding(
            padding: const EdgeInsets.fromLTRB(16, 16, 16, 8),
            child: InkWell(
              onTap: _onIndustrySelectorClicked,
