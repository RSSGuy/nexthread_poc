...
import '../../core/topic_config.dart';
import '../../core/models.dart';
import '../dialogs/simulation_dialog.dart'; // IMPORT NEW FILE
...
class MarketPulseCard extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // 2. MARKET DATA ROW (Supports Single or Multi)
          if (marketFact != null)
  ...
  // --- NEW: Decides whether to show one row or a list ---
  Widget _buildContent(MarketFact fact) {
    if (fact.subFacts.isNotEmpty) {
      // MULTI-BENCHMARK (e.g. Mining)
      return Column(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // ROW 2: Value + Trend + Status
          Row(
            children: [
              Text(fact.value, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), // Increased size slightly
              const Spacer(),
              Row(
                children: [
                  Icon(icon, size: 14, color: trendColor),
...
import '../../core/topic_config.dart';
import '../../core/models.dart';
import '../../core/market_data_provider.dart'; // Needed for Global Fetch
import '../dialogs/simulation_dialog.dart';
...
class MarketPulseCard extends StatelessWidget {
  ...
  // --- NEW: Global Indices Modal ---
  void _showGlobalIndicesDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Padding(
                      padding: const EdgeInsets.only(bottom: 8.0),
                      child: _buildPulseRowStatic(fact), // Reusing styling
                    );
                  },
                );
              },
  ...
  // Helper for static access in Dialog
  static Widget _buildPulseRowStatic(MarketFact fact) {
    final isUp = !fact.trend.startsWith('-');
    final trendColor = isUp ? const Color(0xFF10B981) : const Color(0xFFEF4444);
    final icon = isUp ? Icons.arrow_upward : Icons.arrow_downward;
        ...
        children: [
          Row(
            children: [
              ...
              // NEW: GLOBAL INDICES BUTTON
              InkWell(
                onTap: () => _showGlobalIndicesDialog(context),
                borderRadius: BorderRadius.circular(8),
                child: Container(
                  height: 48,
                  width: 48,
  ...
  Widget _buildContent(MarketFact fact) {
    if (fact.subFacts.isNotEmpty) {
      return Column(
        children: fact.subFacts.map((sub) => Padding(
          padding: const EdgeInsets.only(bottom: 8.0),
          child: _buildPulseRowStatic(sub), // Used static version
        )).toList(),
      );
    } else {
      return _buildPulseRowStatic(fact); // Used static version
    }
  }
}*/
