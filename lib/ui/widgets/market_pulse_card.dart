...
import '../dialogs/simulation_dialog.dart';
import '../dialogs/global_indices_dialog.dart';
import 'market_pulse_row.dart'; // Import the new widget
        ...
        children: [
          Row(
            children: [
              ...
              // GLOBAL INDICES BUTTON
              InkWell(
                onTap: () => GlobalIndicesDialog.show(context, topics),
                borderRadius: BorderRadius.circular(8),
                child: Container(
                  height: 48,
                  width: 48,
  ...
  Widget _buildContent(MarketFact fact) {
    if (fact.subFacts.isNotEmpty) {
      return Column(
        children: fact.subFacts.map((sub) => Padding(
          padding: const EdgeInsets.only(bottom: 8.0),
          child: MarketPulseRow(fact: sub), // Using new widget
        )).toList(),
      );
    } else {
      return MarketPulseRow(fact: fact); // Using new widget
    }
  }
}